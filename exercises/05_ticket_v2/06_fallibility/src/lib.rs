...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            Err::<Ticket, String>("Title cannot be empty".into())
        } else if title.len() > 50 {
            Err::<Ticket, String>("Title cannot be longer than 50 bytes".into())
        } else if description.is_empty() {
            Err::<Ticket, String>("Description cannot be empty".into())
        } else if description.len() > 500 {
            Err::<Ticket, String>("Description cannot be longer than 500 bytes".into())
        } else {
            Ok(Ticket {
                title,
                description,
